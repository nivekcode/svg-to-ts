...
export const generateTypeDefinition = (typeName: string, svgDefinitions: SvgDefinition[]): string => {
  ...
  `;
  const types = svgDefinitions.map(({ typeName }, index) =>
    index === svgDefinitions.length - 1 ? `'${typeName}';` : `'${typeName}'${typesDelimitor}`
  );
